...
public class YouTubeService {
    ...
    @Value("${youtube.api.max.related.videos:3}")
    public int maxRelatedVideos;
    ...
    public SearchVideo searchVideo(String videoTitle){
        ...
        Video primaryVideo=getVideoId(primaryVideoId);

        List<Video> relatedVideos=new ArrayList<>();

        for(String id:relatedVideoIds){
            Video video=getVideoId(id);
    ...
    private Video getVideoId(String videoId){
        VideoApiResponse response=webClientBuilder.baseUrl(baseUrl).build()
                .get()
                .uri(uriBuilder -> uriBuilder
                        .path("/videos")
